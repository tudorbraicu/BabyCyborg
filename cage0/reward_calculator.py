...
class RewardCalculator:
    ...
    def calculate_reward(self, action_result) -> float:
        """
        Calculate reward from an action result.

        Args:
            action_result: ActionResult object from environment execution

        Returns:
            Calculated reward value (0.0 if action failed)
        """
        return action_result.reward if action_result.success else 0.0
